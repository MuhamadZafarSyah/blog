...
@section('content')
    ...
    <main class="max-w-screen-lg mx-auto">
        <h1 class="text-2xl font-serif font-semibold m-4">Page {{ $title }}</h1>
        <!-- component -->
                ...
                <div class="flex w-full">
                    ...
                    <form action="/posts" method="GET" class="flex">
                        ...
                        <input type="text" name="search"
                            class="w-full bg-gray-200 pl-2 text-base font-semibold outline-0"
                            placeholder="Cari judul postingan..." value="{{ request('search') }}" />
        ...
        @if ($posts->count())
            <article class="flex my-8 bg-white transition hover:shadow-none duration-500 shadow-xl">
                <div class="rotate-180 p-2 [writing-mode:_vertical-lr]">
                    <time datetime="2022-10-10"
                        ...
                        <span class="w-px flex-1 bg-gray-900/10"></span>
                        <a
                            href="/posts?category={{ $posts[0]->category->slug }}"><span>{{ $posts[0]->category->name }}</span></a>
                    </time>
                </div>

                <div class="block sm:basis-60 basis-48">
                    @if ($posts[0]->image)
                        <img alt="{{ $posts[0]->category->name }}" src="/storage/{{ $posts[0]->image }}"
                            class="aspect-square h-full md:w-full  object-cover" />
                    @else
                        <img class="rounded-t-lg w-full h-full object-cover"
                            src="https://source.unsplash.com/1200x800?{{ $posts[0]->category->name }}"
                    ...
                    <div class="border-s border-gray-900/10 p-4 sm:border-l-transparent sm:p-6">
                        <a href="/posts/{{ $posts[0]->slug }}">
                            ...
                            <a href="/posts?author={{ $posts[0]->author->username }}"
                                class="font-semibold text-blue-800">By:
            ...
            <div class="grid grid-cols-2 mx-auto md:grid-cols-3 gap-4 w-full  ">
                @foreach ($posts->skip(1) as $post)
                    <div class="bg-white shadow-md border border-gray-200 rounded-lg max-w-[18rem] md:max-w-sm mb-5">
                        ...
                        <div class="p-2 md:p-5">
                            <p>
                                <a href="/posts?author={{ $post->author->username }}"
                                    class="font-semibold text-blue-800 text-xs md:text-base">
                                    By: {{ $post->author->name }}</a> | <a
                                    href="/posts?category={{ $post->category->slug }}"
                                    class="font-semibold text-blue-800 text-xs md:text-base">{{ $post->category->name }}</a>
                            </p>
                            <a>
